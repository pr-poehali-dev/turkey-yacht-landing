...
const YACHT_GLOSSARY = [
  { icon: "⛵", term: "Кокпит", def: "Открытая площадка на корме яхты — ваша «терраса». Здесь штурвал, капитан и большая часть жизни на переходах." },
  { icon: "🛋", term: "Кают-компания", def: "Общий салон внутри яхты — столовая, гостиная и кухня в одном. Тут едят, общаются и отдыхают." },
  { icon: "🍳", term: "Камбуз", def: "Кухня на яхте. Плита, холодильник, раковина и всё для готовки — в компактном пространстве." },
  { icon: "🚽", term: "Гальюн", def: "Туалет на яхте. Работает немного иначе, чем дома — капитан объяснит при посадке." },
  { icon: "⚓", term: "Марина", def: "Яхтенный порт с причалами, водой, электричеством и иногда ресторанами. Здесь начинается и заканчивается маршрут." },
  { icon: "🧭", term: "Бакштаг / Галфвинд", def: "Курс яхты относительно ветра. Новичкам знать необязательно — капитан разберётся сам." },
  { icon: "🪝", term: "Якорная стоянка", def: "Стоянка вдали от берега — яхта держится на якоре. Тишина, звёзды, вода вокруг." },
  { icon: "🏊", term: "Купальная платформа", def: "Площадка в корме на уровне воды — для прыжков и выхода из моря. Лесенка сюда же." },
  { icon: "🚤", term: "Динги", def: "Маленькая надувная лодка с мотором — доставляет на берег, когда яхта стоит на якоре." },
  { icon: "🪢", term: "Шкот", def: "Верёвка (снасть) для управления парусами. Если капитан попросит потянуть — это как раз шкот." },
];
    ...
    <div style={{ background: "var(--sea-deep)", color: "var(--text-primary)", fontFamily: "'Golos Text', sans-serif", minHeight: "100vh" }}>
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-10">
            {[
              ...
              },
              {
                ...
                img: "https://cdn.poehali.dev/projects/281b68c9-e4d3-42d4-bf37-8d9d27e5e4e9/bucket/3315bd08-273c-4ff7-bc11-22fe769b27b8.png",
              },
              {
                icon: "🍳",
                title: "Камбуз — кухня на борту",
                desc: "Полноценная кухня прямо на яхте. Газовая плита, раковина, холодильник, вся посуда. Можно готовить самим или заказать повара на весь маршрут. Свежий улов — сразу на сковороду.",
                img: "https://cdn.poehali.dev/projects/281b68c9-e4d3-42d4-bf37-8d9d27e5e4e9/bucket/3ed02feb-98cc-4b46-af74-3a76f9e7163f.jpg",
              },
              {
                icon: "⛵",
          ...
          <div className="grid md:grid-cols-2 gap-3 reveal delay-200">
            {YACHT_GLOSSARY.map((item) => (
              <div key={item.term} className="flex gap-4 p-4 rounded-xl" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(38,201,195,0.08)" }}>
                <div className="flex-shrink-0 flex items-center gap-2 px-3 py-1 rounded-lg h-fit" style={{ background: "rgba(38,201,195,0.12)", border: "1px solid rgba(38,201,195,0.2)" }}>
      ...
      <section id="gallery" className="py-24 relative" style={{ background: "linear-gradient(180deg, #163054 0%, #0d1f3c 100%)" }}>
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          {/* Main slider */}
          <div className="relative rounded-3xl overflow-hidden mb-4 reveal" style={{ height: "clamp(320px, 55vw, 600px)", border: "1px solid rgba(38,201,195,0.15)", background: "rgba(13,24,48,0.95)" }}>
            <img
              src={GALLERY_ITEMS[gallerySlide].img}
              alt={GALLERY_ITEMS[gallerySlide].label}
              className="w-full h-full transition-all duration-500"
              style={{
                objectFit: ("fit" in GALLERY_ITEMS[gallerySlide] && GALLERY_ITEMS[gallerySlide].fit === "contain") ? "contain" : "cover",
                objectPosition: GALLERY_ITEMS[gallerySlide].pos,
              }}
            />
        ...
        <div className="max-w-5xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 gap-5 mb-8">
            {[
              {
                icon: "🧭",
                title: "Капитан — не наёмный сотрудник, а хозяин",
              ...
              },
              {
                ...
                desc: "Цена фиксируется заранее. Что входит в стоимость — написано явно, без мелкого шрифта. Вы платите ровно столько, сколько договорились. Предоплата фиксирует место, остаток — при посадке.",
                accent: false,
              },
            ].map((item) => (
